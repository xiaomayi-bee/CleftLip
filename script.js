...
// 缩略图相关变量
let folderImages = []; // 文件夹中的所有图片
let currentFolderIndex = -1; // 当前图片索引
let folderPatientId = ''; // 文件夹提取的病历号
let currentImageFile = null; // 当前图片文件对象

// 标点模式状态
let isPointMode = false;
let currentSelectedPointName = null; // 新增：当前选择的点名称
...
function bindEventListeners() {
    ...
    // 缩放控制
    document.getElementById('zoomInBtn').addEventListener('click', zoomInAtCenter);
    document.getElementById('zoomOutBtn').addEventListener('click', zoomOutAtCenter);
    ...
    canvas.addEventListener('wheel', handleWheelWithMouseCenter);
    ...
    // 表格行点击事件（委托）
    document.getElementById('pointsTableBody').addEventListener('click', (e) => {
        if (e.target.tagName === 'TD' || e.target.tagName === 'TR') {
            const row = e.target.closest('tr');
            if (row) {
                // 更新选中的表格行
                updateSelectedTableRow(row);

                // 更新选中点显示
                document.getElementById('selectedPoint').textContent = `当前选择: ${currentSelectedPointName}`;

                console.log(`手动选择了点: ${currentSelectedPointName}`);

                // 注意：这里移除了自动跳转逻辑
                // 手动选择点时不会自动跳转，只有在添加或拖动标点后才会跳转
            }
        }
    });
...
// 更新选中的表格行
function updateSelectedTableRow(selectedRow) {
    // 移除所有行的选中状态
    ...
    // 保存当前选择的点名称
    currentSelectedPointName = selectedRow.cells[0].textContent;
...
function handleFolderUpload(event) {
    ...
    // 提取病历号
    const extractionResult = extractPatientIdFromPath(folderPath);
    folderPatientId = extractionResult.patientId || '';

    // 保存文件夹图片信息
    folderImages = imageFiles.map(file => ({
        file: file,
        name: file.name,
        url: URL.createObjectURL(file),
        patientId: folderPatientId // 所有图片使用相同的病历号
    }));

    // 显示第一张图片
    if (folderImages.length > 0) {
        currentFolderIndex = 0;
        loadFolderImage(currentFolderIndex);
        ...
        console.log(`已加载文件夹，共${folderImages.length}张图片，病历号: ${folderPatientId}`);
    }
}

// 加载文件夹中的图片
function loadFolderImage(index) {
    if (index < 0 || index >= folderImages.length) {
        return;
    }

    const imageItem = folderImages[index];
    currentFolderIndex = index;
    currentImageFile = imageItem.file; // 保存当前图片文件
    ...
    img.onload = () => {
        ...
        // 自动填充病历号（如果从文件夹中提取到了）
        if (folderPatientId) {
            patientInfo.patientId = folderPatientId;
        ...
        // 如果是标点模式，自动选择第一个点
        if (isPointMode) {
            autoSelectFirstAvailablePoint();
        }

        console.log(`已加载第${index + 1}张图片: ${imageItem.name}`);
    };

    img.onerror = () => {
        alert(`图片加载失败: ${imageItem.name}`);
        // 尝试加载下一张
        if (index + 1 < folderImages.length) {
            setTimeout(() => loadFolderImage(index + 1), 100);
        }
    };
...
// 自动选择第一个可用的点
function autoSelectFirstAvailablePoint() {
    const rows = document.querySelectorAll('#pointsTableBody tr');

    // 如果之前有选择的点，尝试从那个点开始
    if (currentSelectedPointName) {
        for (let i = 0; i < rows.length; i++) {
            if (rows[i].cells[0].textContent === currentSelectedPointName) {
                updateSelectedTableRow(rows[i]);
                document.getElementById('selectedPoint').textContent = `当前选择: ${currentSelectedPointName}`;
                return;
            }
        }
    ...
    // 否则选择第一个点
    if (rows.length > 0) {
        const firstPointName = rows[0].cells[0].textContent;
        updateSelectedTableRow(rows[0]);
        document.getElementById('selectedPoint').textContent = `当前选择: ${firstPointName}`;
        currentSelectedPointName = firstPointName;
    }
}
...
function resetTableSelection() {
    ...
    // 如果是标点模式，自动选择第一个点
    if (isPointMode) {
        autoSelectFirstPoint();
    }
}

// 自动选择第一个点
function autoSelectFirstPoint() {
    const rows = document.querySelectorAll('#pointsTableBody tr');
    if (rows.length > 0) {
        rows[0].classList.add('selected');
        const pointName = rows[0].cells[0].textContent;
        document.getElementById('selectedPoint').textContent = `当前选择: ${pointName}`;
...
function updateThumbnails() {
    ...
    folderImages.forEach((item, index) => {
...
function updateCurrentImageIndex() {
    ...
    if (currentIndexElement) {
        currentIndexElement.textContent = `${currentFolderIndex + 1}/${folderImages.length}`;
    }
}

// 更新文件夹信息显示
function updateFolderInfo() {
    const folderInfoElement = document.getElementById('folderInfo');
    if (folderInfoElement) {
        if (folderImages.length > 0) {
            folderInfoElement.textContent = `共${folderImages.length}张图片 | 病历号: ${folderPatientId || '未识别'}`;
        } else {
            folderInfoElement.textContent = '未加载文件夹';
        }
    }
}
...
function showPrevImage() {
    ...
    const newIndex = (currentFolderIndex - 1 + folderImages.length) % folderImages.length;
    loadFolderImage(newIndex);
}
...
function showNextImage() {
    ...
    const newIndex = (currentFolderIndex + 1) % folderImages.length;
    loadFolderImage(newIndex);
}
...
function updateShortcutHint() {
    ...
    if (hintElement) {
        if (isPointMode) {
            hintElement.textContent = '空格键切换标点 | Tab键下一张 | Shift+Tab上一张 | 点击标点可拖动';
        } else {
            hintElement.textContent = '空格键切换标点 | Tab键下一张 | Shift+Tab上一张';
        }
    }
}

// 自动选择下一个点
function autoSelectNextPoint(currentPointName = null) {
    if (!isPointMode || !autoJumpEnabled) return;
    ...
    if (currentPointName) {
        for (let i = 0; i < rows.length; i++) {
            if (rows[i].cells[0].textContent === currentPointName) {
                startIndex = i + 1;
                break;
            }
        }
    ...
    for (let i = startIndex; i < rows.length; i++) {
        const pointName = rows[i].cells[0].textContent;
        const existingPoint = points.find(p => p.name === pointName);

        // 如果点不存在或者标记为不存在，则选择这个点
        if (!existingPoint || !existingPoint.exists) {
            // 更新选中的表格行
            updateSelectedTableRow(rows[i]);
            ...
            console.log(`自动选择下一个点: ${pointName}`);
            return true; // 找到了点
        }
    }
    ...
    for (let i = 0; i < startIndex; i++) {
        ...
        if (!existingPoint || !existingPoint.exists) {
            ...
            console.log(`从开头重新选择点: ${pointName}`);
            return true; // 找到了点
        }
    }
    ...
    if (rows.length > 0) {
        ...
        updateSelectedTableRow(rows[0]);
        document.getElementById('selectedPoint').textContent = `当前选择: ${firstPointName}`;
        console.log(`所有点已标记，选择第一个点: ${firstPointName}`);
    }

    return false; // 没有找到未标记的点
}
...
function handleMouseUp(e) {
    if (isDragging && isPointMode && selectedPointIndex !== -1) {
        // 如果是在标点模式下拖动标点，拖动结束后自动选择下一个点
        const point = points[selectedPointIndex];
        if (point) {
            // 自动选择下一个点
            autoJumpToNextPoint(point.name);
        }
    }
...
// 处理鼠标滚轮事件 - 以光标位置为中心缩放
function handleWheelWithMouseCenter(e) {
    if (!image) return;
    ...
    // 计算鼠标在图像上的相对位置
    const mouseImgX = (mouseX - imgX) / imgWidth;
    const mouseImgY = (mouseY - imgY) / imgHeight;
    ...
    // 调整偏移量，使鼠标点保持在同一位置
    offsetX = mouseX - mouseImgX * newImgWidth - (canvas.width - newImgWidth) / 2;
    offsetY = mouseY - mouseImgY * newImgHeight - (canvas.height - newImgHeight) / 2;
...
function addPointAtPosition(x, y) {
    ...
    currentSelectedPointName = pointName; // 保存当前选择的点
    ...
    if (existingPointIndex !== -1) {
        ...
        // 设置选中的标点索引
        selectedPointIndex = existingPointIndex;
    } else {
        // 创建新标点
        const newPoint = {
            name: pointName,
...
function selectPoint(index) {
    ...
    // 保存当前选择的点名称
    currentSelectedPointName = point.name;
...
function updatePointPosition(index, mouseX, mouseY) {
    ...
    // 拖动标点后自动选择下一个点
    autoJumpToNextPoint(points[index].name);
}
...
// 自动跳转到下一个点（简单顺序跳转）
function autoJumpToNextPoint(currentPointName) {
    ...
    // 如果没找到当前点，从第一个开始
    if (nextIndex === -1 && rows.length > 0) {
        nextIndex = 0;
    }

    // 跳转到下一个点
    if (nextIndex !== -1 && nextIndex < rows.length) {
        const nextPointName = rows[nextIndex].cells[0].textContent;
        updateSelectedTableRow(rows[nextIndex]);
        document.getElementById('selectedPoint').textContent = `当前选择: ${nextPointName}`;
        currentSelectedPointName = nextPointName;

        console.log(`自动跳转到下一个点: ${nextPointName}`);
    }
}
...
function saveImageFile() {
    ...
    // 使用当前图片文件
    const currentFile = currentImageFile;
...
function exportAnnotation() {
    ...
    const imageInfo = {
        ...
        format: currentImageFile ? currentImageFile.type : 'unknown',
        file_name: currentImageFile ? currentImageFile.name : 'unknown',
        file_size: currentImageFile ? currentImageFile.size : 0,
...
function handleKeyDown(e) {
    ...
    // Tab键：下一张图片
    if (e.key === 'Tab' && folderImages.length > 0) {
        e.preventDefault();
        if (e.shiftKey) {
            showPrevImage();
        } else {
...
// 实现平滑缩放动画效果（以鼠标位置为中心）
function smoothZoomAtMouse(targetScale, mouseX, mouseY, duration = 200) {
    if (!image) return;
    ...
    // 计算当前鼠标在图像坐标系中的位置
    const startImgWidth = image.width * startScale;
    const startImgHeight = image.height * startScale;
    const startImgX = (canvas.width - startImgWidth) / 2 + startOffsetX;
    const startImgY = (canvas.height - startImgHeight) / 2 + startOffsetY;

    // 确保鼠标在图像范围内
    if (mouseX < startImgX || mouseX > startImgX + startImgWidth ||
        mouseY < startImgY || mouseY > startImgY + startImgHeight) {
        // 如果不在图像范围内，使用中心点缩放
        return smoothZoom(targetScale, startOffsetX, startOffsetY, duration);
    }

    // 计算鼠标在图像上的相对位置
    const mouseImgX = (mouseX - startImgX) / startImgWidth;
    const mouseImgY = (mouseY - startImgY) / startImgHeight;

    // 计算新尺寸
    const newImgWidth = image.width * targetScale;
    const newImgHeight = image.height * targetScale;

    // 计算目标偏移量
    const targetOffsetX = mouseX - mouseImgX * newImgWidth - (canvas.width - newImgWidth) / 2;
    const targetOffsetY = mouseY - mouseImgY * newImgHeight - (canvas.height - newImgHeight) / 2;
...
// 缩放控制函数 - 以鼠标位置为中心
function zoomInAtMouse(mouseX, mouseY) {
    if (!image) return;
    ...
    // 使用平滑动画进行缩放
    smoothZoomAtMouse(newScale, mouseX, mouseY);
}

function zoomOutAtMouse(mouseX, mouseY) {
    if (!image) return;
...
function zoomOutAtCenter() {
    ...
    zoomOutAtMouse(centerX, centerY);
}
